...
decodifica. Los modelos de varios colores listan sus piezas en «partes».
Uso: python visor/preparar.py
"""
import base64, json, pathlib

aqui = pathlib.Path(__file__).parent
for m in json.loads((aqui / "modelos.json").read_text()):
    for p in m.get("partes", [m]):
        origen = aqui.parent / p["origen"]
        destino = aqui / p["archivo"]
